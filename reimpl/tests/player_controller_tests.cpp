...
void test_jump_while_moving_left_and_right() {
    ...
    expect(left_state.player.x_vel == -2, "left+jump should set negative x velocity");
    expect(left_state.player.y_vel == -5, "left+jump should apply jump impulse");
    expect(left_state.player.jump_counter == 0, "left+jump should consume jump counter");
    ...
    expect(right_state.player.x == 2, "right+jump should move right by walk step");
    expect(right_state.player.x_vel == 2, "right+jump should set positive x velocity");
    expect(right_state.player.y_vel == -5, "right+jump should apply jump impulse");
    expect(right_state.player.jump_counter == 0, "right+jump should consume jump counter");
}
...
void test_short_hop_vs_full_jump_counter_usage() {
    ...
    comic2::apply_airborne_physics_tick(
        short_hop, comic2::PlayerMotionConfig{}, comic2::TileCollisionConfig{});
    ...
    comic2::apply_airborne_physics_tick(
        full_jump, comic2::PlayerMotionConfig{}, comic2::TileCollisionConfig{});

    expect(full_jump.player.jump_counter == 0,
           "holding jump during airborne rise should consume additional jump counter");
}
...
void test_ledge_walk_off_transitions_into_fall() {
    ...
    comic2::apply_input_tick(state, comic2::PlayerMotionConfig{});
    expect(state.player.x == 6, "input tick should move player past platform edge");

    comic2::apply_grounded_physics_tick(state, comic2::PlayerMotionConfig{}, collision);

    expect(state.player.is_airborne, "walking off ledge should enter airborne state");
    expect(state.player.is_physics_active, "walking off ledge should keep physics active");
    expect(state.player.y_vel > 0, "walking off ledge should start downward fall velocity");
}
