...
void test_signed_rle_decode() {
    ...
    // 0x00 -> terminator
    const std::vector<std::uint8_t> encoded = {0x03, 0x10, 0x11, 0x12, 0xFD, 0xAA, 0x00};
    const auto out = comic2::decode_signed_rle(encoded);

    const std::vector<std::uint8_t> expected = {0x10, 0x11, 0x12, 0xAA, 0xAA, 0xAA};
    expect(out.bytes == expected, "decode_signed_rle produced unexpected bytes");
    expect(out.consumed == encoded.size(), "decode_signed_rle consumed byte count mismatch");
}
...
void test_ega_4plane_decode() {
    ...
    expect((image.planes[0] == std::vector<std::uint8_t>{0x01, 0x02}), "plane 0 mismatch");
    expect((image.planes[1] == std::vector<std::uint8_t>{0x03, 0x03}), "plane 1 mismatch");
    expect((image.planes[2] == std::vector<std::uint8_t>{0x04, 0x05}), "plane 2 mismatch");
    expect((image.planes[3] == std::vector<std::uint8_t>{0x06, 0x06}), "plane 3 mismatch");
}
