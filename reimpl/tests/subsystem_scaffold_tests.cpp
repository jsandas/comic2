...
void test_ent_build_room_entity_list_filters_by_room() {
    std::vector<comic2::MappedObject12> mapped_objects = {
        {.room_x = 0, .room_y = 0, .descriptor_ptr = 0x100, .state_flags = 0, .world_x = 50, .world_y = 100},
        {.room_x = 1, .room_y = 0, .descriptor_ptr = 0x200, .state_flags = 0, .world_x = 150, .world_y = 100},
        {.room_x = 0, .room_y = 0, .descriptor_ptr = 0x300, .state_flags = 0, .world_x = 250, .world_y = 100},
    };

    std::vector<comic2::ActiveEntity8> active_entities;
    comic2::EntityActivationState activation_state;
    comic2::ent_build_room_entity_list(mapped_objects, 0, 0, active_entities, activation_state);

    expect_eq(active_entities.size(), 2, "should find 2 entities in room (0,0)");
    expect(active_entities[0].x == 50 && active_entities[0].y == 100, "first entity position mismatch");
    expect(active_entities[1].x == 250 && active_entities[1].y == 100, "second entity position mismatch");
...
void test_ent_build_runtime_slots_for_viewport_culls() {
    ...
    std::vector<comic2::MappedObject12> mapped_objects = {
        {.room_x = 16, .room_y = 16, .descriptor_ptr = 0x100, .state_flags = 0, .world_x = 10, .world_y = 10},
        {.room_x = 16, .room_y = 16, .descriptor_ptr = 0x200, .state_flags = 0, .world_x = 500, .world_y = 10},
        {.room_x = 16, .room_y = 16, .descriptor_ptr = 0x300, .state_flags = 0, .world_x = 100, .world_y = 10},
    };
...
void test_spawn_player_projectile_with_facing() {
    ...
    expect(projectiles[0].y == 58, "projectile y should be player y + 8 when grounded");
    expect(projectiles[0].x_vel == 0x0010, "projectile x_vel should be positive when facing right");
    expect(projectiles[0].y_vel == -0x000C, "projectile y_vel should be negative (upward arc)");
...
void test_spawn_player_projectile_airborne_adjustment() {
    ...
    expect_eq(projectiles.size(), 1, "should spawn one projectile");
    expect(projectiles[0].x_vel == 0, "projectile x_vel should be 0 when facing left and airborne (adjustment cancels)");
    expect(projectiles[0].y == 54, "projectile y should be player y + 4 when airborne");
}
...
void test_projectile_tile_collision_detection() {
    ...
    const bool collision = comic2::check_projectile_tile_collision(projectile, room_grid);
...
void test_projectile_viewport_culling() {
    ...
    comic2::update_projectiles(projectiles, comic2::ProjectileBounds{.min_x = 0, .max_x = 319, .min_y = 0, .max_y = 199},
                               empty_grid, 100, 0);

    expect(!projectiles[0].active, "projectile should deactivate when outside viewport bounds (x < viewport_min_x - 8)");
}
