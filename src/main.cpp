...
int main(int argc, char** argv) {
    try {
        ...
        const std::vector<std::string> frpak_files = {
            "FRPAK.001", "FRPAK.002", "FRPAK.003", "FRPAK.004", "FRPAK.005", "FRPAK.006", "FRPAK.007",
        };
        ...
        for (const auto& name : frpak_files) {
            ...
            std::cout << name << ": row_span=0x" << std::hex << image.row_span_bytes << std::dec
                      << " plane_bytes=" << image.planes[0].size() << "\n";
        }
