...
void test_decode_known_rle_4plane_stream() {
  ...
  for (std::size_t p = 0; p < 4; ++p) {
    auto result = comic2::validation::compare_plane_bytes(image.planes[p],
                                                          expected[p], p);
...
void test_opaque_blit_solid_image_to_empty_surface() {
  ...
  auto result = comic2::validation::validate_opaque_blit(surface, image, 0, 0,
                                                         expected_planes);
...
void test_opaque_blit_at_offset() {
  ...
  auto result = comic2::validation::validate_opaque_blit(surface, image, 16, 10,
                                                         expected_planes);
  expect_empty(result, "opaque_blit_at_offset");

  // Verify surrounding bytes are still zero
  expect(surface.get_plane_byte(0, 1, 10) == 0,
         "byte before blit region should be 0");
  expect(surface.get_plane_byte(0, 1, 17) == 0,
         "byte after blit region should be 0");
}
...
void test_opaque_blit_overwrites_existing_data() {
  ...
  expect(surface.get_plane_byte(0, 0, 8) == 0xFF,
...
void test_masked_blit_or_with_empty_surface() {
  ...
  auto result = comic2::validation::validate_masked_blit(surface, image, 0, 0,
                                                         expected_planes);
...
void test_masked_blit_or_with_existing_data() {
  ...
  // Outside blit region should be unchanged
  expect(surface.get_plane_byte(0, 0, 8) == 0xF0,
         "outside blit region should be unchanged");
}
...
void test_masked_blit_double_or_idempotent() {
  ...
  for (std::size_t p = 0; p < 4; ++p) {
    for (std::size_t row = 0; row < 8; ++row) {
      expect(surface1.get_plane_byte(p, 0, row) ==
                 surface2.get_plane_byte(p, 0, row),
...
void test_compare_plane_region_exact_match() {
  ...
  auto result = comic2::validation::compare_plane_region(surface, 0, 5, 10, 2,
                                                         2, expected);
...
void test_compare_plane_region_mismatch() {
  ...
  auto result = comic2::validation::compare_plane_region(surface, 0, 5, 10, 1,
                                                         1, expected);
...
void test_multi_blit_composition() {
  ...
  auto image1 =
      comic2::validation::make_8x8_solid_image(0x01, 0x00, 0x00, 0x00);
  auto image2 =
      comic2::validation::make_8x8_solid_image(0x02, 0x00, 0x00, 0x00);
  ...
  auto result1 =
      comic2::validation::compare_plane_region(surface, 0, 0, 0, 1, 8, exp1);
  expect_empty(result1, "first blit region");

  // Region 2 should have 0x02
  std::vector<std::uint8_t> exp2(8, 0x02);
  auto result2 =
      comic2::validation::compare_plane_region(surface, 0, 2, 0, 1, 8, exp2);
...
void test_patterned_image_blit() {
  ...
  expect(surface.get_plane_byte(0, 0, 0) == 1,
         "patterned image plane 0 byte 0");
  expect(surface.get_plane_byte(0, 1, 0) == 2,
         "patterned image plane 0 byte 1");

  // Plane 3: byte[i] = (3+1) * (i+1) = 4*(i+1)
  expect(surface.get_plane_byte(3, 0, 0) == 4,
         "patterned image plane 3 byte 0");
  expect(surface.get_plane_byte(3, 1, 0) == 8,
...
void run_renderer_validation_tests() {
  ...
  test_frpak_decode_on_demand_caches_result();
  test_frpak_decode_invalid_record_id_fails();
  test_frpak_decode_rejects_out_of_range_record_metadata();
}
